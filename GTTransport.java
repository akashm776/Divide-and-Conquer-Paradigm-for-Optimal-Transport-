        ...
        if (minIndex < n) {
          ...
          for (int a = 0; a < n; a++) {
            if (capacityBA[minIndex][a] > 0) {
              int aIndex = a + n;
              int newDist =
                lv[minIndex] + CBA[minIndex][a] + 1 - y[minIndex] - y[aIndex];
      ...
      for (int vertex = 0; vertex < n; vertex++) {
        if (bFree[vertex]) {
          ...
          while (deficiencyB[vertex] > 0 && vertexVisited[vertex] < 2 * n - 1) {
            ...
            int beta = Integer.min(
              deficiencyB[ap.get(0)],
              deficiencyA[ap.get(ap.size() - 1) - n]
            );
    ...
    while (!path.isEmpty()) {
      ...
      for (int i = rangeStart; i < rangeEnd; i++) {
        ...
        if (end < n) {
          ...
          int a = i - n;
          if (
            CBA[end][a] + 1 - y[end] - y[a + n] == 0 && capacityBA[end][a] > 0
          ) {
            backtrack = false;
            //Add vertex to path
            path.add(i);
